var LC3 = function() {
    ...
    // YCH: SSP when in user mode, USP when in superviser mode
    // see Patt & Patel Appendix C Figure C.7 states 45 and 59
    this.osp = 0x2F00;
...
LC3.prototype.execute = function(op, address, operand) {
    ...
    switch (op.opcode) {
		...
		// YCH: Patt and Patel Figure C.7 State 59
		if (this.psr & 0x8000) {
		    r6 = this.r[6];
		    this.setRegister(6, this.osp);
		    this.osp = r6;
		}
            }
            return null;
...
LC3.prototype.writeMemory = function(address, data) {
    ...
    // YCH: don't let the user change the DSR sign bit.
    if (address === this.dsr) {
        this.setMemory(this.dsr, (data & 0x7FFF) | (this.getMemory(this.dsr) & 0x8000));
	return;
    }
...
LC3.prototype.checkInterrupts = function() {
    ...
    if ((this.getMemory(this.kbsr) & interruptMask) === interruptMask) {
        this.interrupt(this.kbpl, this.kbiv);
    }
    // Check the display
    if ((this.getMemory(this.dsr) & interruptMask) === interruptMask) {
        this.interrupt(this.dpl, this.div);
    }

    var clock = this.getMemory(this.clsr) | (clockInterrupt << 15);
    if ((clock & interruptMask) === interruptMask) {
	this.interrupt(this.clockpl, this.clockiv);
    }
};

LC3.prototype.interrupt = function(priorityLevel, vector) {
    ...
    if (vector == this.clockiv) {
	clockInterrupt = 0;
    }

    // Get supervisor stack pointer
    // YCH: if we're not in supervisor mode, swap it over.
    // State 45 from Patt and Patel Figure C.7
    var ssp = this.getRegister(6);
    if (this.psr & 0x8000) {
	ssp = this.osp;
	this.osp = this.getRegister(6);
    }
    ...
    // Set new PC
    this.setRegister('pc', this.getMemory(vector));
